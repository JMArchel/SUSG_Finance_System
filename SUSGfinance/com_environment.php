...
//firstexpense
$firstsemexpenses=mysqli_query($conn,"SELECT SUM(`amount`) AS total_amount FROM `com_environment` WHERE 1=SUBSTRING(`internal_number`,LOCATE('-',`internal_number`)+1,1);");
$firstsemexpenses = mysqli_fetch_array($firstsemexpenses);
$firstsemexpenses = $firstsemexpenses['total_amount'];

//secondexpense
$secondsemexpenses=mysqli_query($conn,"SELECT SUM(`amount`) AS total_amount FROM `com_environment` WHERE 2=SUBSTRING(`internal_number`,LOCATE('-',`internal_number`)+1,1);");
    ...
    <div class="row d-flex justify-content-center">
        <div class="col-6">
            <table class="table table-borderless table-striped">
                    ...
                    <tbody>
                        <?php 
                            $environment=mysqli_query($conn,"SELECT `id`,`internal_number`,`description`,`amount` FROM com_environment;");
