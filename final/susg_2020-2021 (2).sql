...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Jun 06, 2023 at 05:54 AM
...
--
-- Stand-in structure for view `cash_advances_view`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `cash_advances_view`;
CREATE TABLE IF NOT EXISTS `cash_advances_view` (
`internal_number` varchar(20)
,`ca_amount` decimal(11,2)
);
...
(2, 'CA2020-1-001', 'TeaCHEERS Dayssss\r\n', '200.00');
...
(8, 'CA2020-2-002', 'Clean and Green Revolution 2\r\n', '4290.00'),
...
--
-- Structure for view `cash_advances_view`
--
DROP TABLE IF EXISTS `cash_advances_view`;

DROP VIEW IF EXISTS `cash_advances_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `cash_advances_view`  AS SELECT `cash_advances`.`internal_number` AS `internal_number`, `cash_advances`.`ca_amount` AS `ca_amount` FROM `cash_advances` ;
COMMIT;
