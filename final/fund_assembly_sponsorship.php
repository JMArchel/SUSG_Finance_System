...
//firstexpense
$firstsemexpenses=mysqli_query($conn,"SELECT SUM(`amount`) AS total_amount FROM committee_fund WHERE comm_fund='assembly sponsorship' AND 1=SUBSTRING(`internal_number`,LOCATE('-',`internal_number`)+1,1);");
$firstsemexpenses = mysqli_fetch_array($firstsemexpenses);
$firstsemexpenses = $firstsemexpenses['total_amount'];

//secondexpense
$secondsemexpenses=mysqli_query($conn,"SELECT SUM(`amount`) AS total_amount FROM committee_fund WHERE comm_fund='assembly sponsorship' AND 2=SUBSTRING(`internal_number`,LOCATE('-',`internal_number`)+1,1);");
...
<head>
    ...
    <title>Assembly Sponsorship Fund</title>
</head>
<?php include("sidebar.php"); ?>
<body class="body-top">
<h5 class="title-name fixed-top">Assembly Sponsorship Fund</h5>
    ...
    <div class="row d-flex justify-content-center">
        <div class="col-6">
            <table class="table table-borderless table-striped">
                    ...
                    <tbody>
                        <?php 
                            $assemblysponsorship=mysqli_query($conn,"SELECT `id`,`internal_number`,`description`,`amount` FROM committee_fund WHERE comm_fund='assembly sponsorship';");
                            while ($row=mysqli_fetch_array($assemblysponsorship))
                            {
                                echo "<tr>";
